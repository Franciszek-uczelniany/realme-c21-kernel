...
struct app_monitor_ul_report_msg_st {
	...
	struct app_monitor_retrans_msg_st
		app_retrans_info[TOP_APP_NUM]; /*for retrans static*/
};
...
static void print_report_info(struct app_monitor_ul_report_msg_st *report_msg)
{
	...
	printk("[app_monitor]: report_info:beat_app_count=%u, push_app_count=%u, traffic_app_count=%u, retrans_app_count=%u\n",
	       report_msg->beat_app_count, report_msg->push_app_count,
	       report_msg->traffic_app_count, report_msg->retrans_app_count);
	printk("[app_monitor]:sys_total_packet=%llu,sys_send_packet=%llu,sys_recv_packet=%llu,sys_retrans_total_packet=%llu, sys_retrans_syn_packet=%llu\n",
	       report_msg->sys_total_packets, report_msg->sys_send_packets,
	       report_msg->sys_recv_packets,
	...
	printk("[app_monitor]:sys_total_byte=%llu,sys_send_byte=%llu,sys_recv_byte=%llu\n",
	       report_msg->sys_total_bytes, report_msg->sys_send_bytes,
	       report_msg->sys_recv_bytes);
...
void oplus_app_monitor_fill_beat_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM;  j++) {
			if (top_app_beat_info.read_flag[j]
					|| top_app_beat_info.app_beat_info[j].beat_period == 0) {
				continue;
			}
		...
		if (k < TOP_APP_NUM) {
			report_msg->app_beat_info[i].uid_val =
				top_app_beat_info.app_beat_info[k].uid_val;
			report_msg->app_beat_info[i].beat_period =
				top_app_beat_info.app_beat_info[k].beat_period;
			report_msg->app_beat_info[i].beat_count =
				top_app_beat_info.app_beat_info[k].beat_count;
...
void oplus_app_monitor_fill_push_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM;  j++) {
			if (top_app_push_info.read_flag[j]
					|| top_app_push_info.app_push_info[j].push_period == 0) {
				continue;
			}
		...
		if (k < TOP_APP_NUM) {
			report_msg->app_push_info[i].uid_val =
				top_app_push_info.app_push_info[k].uid_val;
			report_msg->app_push_info[i].push_period =
				top_app_push_info.app_push_info[k].push_period;
			report_msg->app_push_info[i].push_count =
				top_app_push_info.app_push_info[k].push_count;
...
void oplus_app_monitor_fill_traffic_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM;  j++) {
			if (top_app_traffic_info.read_flag[j]
					|| top_app_traffic_info.app_traffic_info[j].total_bytes == 0) {
				continue;
			}
		...
		if (k < TOP_APP_NUM) {
			report_msg->app_traffic_info[i].uid_val =
				top_app_traffic_info.app_traffic_info[k].uid_val;
			report_msg->app_traffic_info[i].total_bytes =
				top_app_traffic_info.app_traffic_info[k].total_bytes;
			report_msg->app_traffic_info[i].total_packets =
				top_app_traffic_info.app_traffic_info[k].total_packets;
			report_msg->app_traffic_info[i].send_bytes =
				top_app_traffic_info.app_traffic_info[k].send_bytes;
			report_msg->app_traffic_info[i].send_packets =
				top_app_traffic_info.app_traffic_info[k].send_packets;
			report_msg->app_traffic_info[i].recv_bytes =
				top_app_traffic_info.app_traffic_info[k].recv_bytes;
			report_msg->app_traffic_info[i].recv_packets =
				top_app_traffic_info.app_traffic_info[k].recv_packets;
...
void oplus_app_monitor_fill_retrans_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM;  j++) {
			if (top_app_retrans_info.read_flag[j]
					|| top_app_retrans_info.app_retrans_info[j].retrans_total_packets == 0) {
				continue;
			}

			if (!max_retrans
					|| max_retrans <
					top_app_retrans_info.app_retrans_info[j].retrans_total_packets) {
		...
		if (k < TOP_APP_NUM) {
			report_msg->app_retrans_info[i].uid_val =
				top_app_retrans_info.app_retrans_info[k].uid_val;
			report_msg->app_retrans_info[i].app_retrans_total_packets =
				top_app_retrans_info.app_retrans_info[k].retrans_total_packets;
			report_msg->app_retrans_info[i].app_retrans_syn_packets =
				top_app_retrans_info.app_retrans_info[k].retrans_syn_packets;
...
extern int apps_monitor_netlink_send_to_user(int msg_type, char *payload,
		int payload_len);
void oplus_app_monitor_send_app_info_report(void)
{
	int ret;
	char msg_buf[1024] = {0};
	struct app_monitor_ul_report_msg_st *report_msg  = (struct
			app_monitor_ul_report_msg_st *)msg_buf;
	...
	if (!app_monitor_start
			|| before((u32)now_tv.tv_sec ,
				  (u32)(REPORT_INTERVAL + last_report_tv.tv_sec))) {
	...
	report_msg->sys_retrans_total_packets =
	...
	ret = apps_monitor_netlink_send_to_user(APPS_POWER_MONITOR_MSG_UL_INFO,
						(char *) msg_buf, report_msg->msg_len);
...
static void report_timer_init(void)
{
	...
	timer_setup(&apps_monitor_report_timer,
		    (void *)apps_monitor_report_timer_function, 0);
}
	...
	if (app_monitor_enable == 1) {
		...
		for (i = 0; i < APP_MONITOR_HASHTABLE_SIZE; i++) {
			...
			while (this_node != head) {
				...
				list_del(this_node);
				app_node_info = list_entry(this_node, struct app_monitor_app_info_list_node_st,
							   list_node);
				kfree(app_node_info);
				this_node = next_node;
			}
...
void oplus_app_monitor_update_top_trafffic_app(struct app_monitor_app_info_st *app_info)
{
	...
	if (top_app_traffic_info.real_app_num < TOP_APP_NUM) {
		top_app_traffic_info.app_traffic_info[top_app_traffic_info.real_app_num] =
			*app_info;
	...
	if (app_info->total_bytes >
			top_app_traffic_info.app_traffic_info[min_index].total_bytes) {
	...
	for (i = 0; i < TOP_APP_NUM; i++) {
		...
		} else if (top_app_traffic_info.app_traffic_info[min_index].total_bytes >
				top_app_traffic_info.app_traffic_info[i].total_bytes) {
			min_index = i;
		}
	}
...
void oplus_app_monitor_update_top_retrans_app(struct app_monitor_app_info_st *app_info)
{
	...
	if (top_app_retrans_info.real_app_num < TOP_APP_NUM) {
		top_app_retrans_info.app_retrans_info[top_app_retrans_info.real_app_num] =
			*app_info;
	...
	/*compare with the min record*/
	if (app_info->retrans_total_packets >
			top_app_retrans_info.app_retrans_info[min_index].retrans_total_packets) {
	...
	for (i = 0; i < TOP_APP_NUM; i++) {
		if (i == 0) {
			...
			top_app_retrans_info.app_retrans_info[min_index].retrans_total_packets >
			top_app_retrans_info.app_retrans_info[i].retrans_total_packets) {
			min_index = i;
		}
	}
...
void oplus_app_monitor_update_top_beat_app(struct app_monitor_app_info_st *app_info)
{
	...
	/*compare with the min record*/
	if (app_info->beat_count >
			top_app_beat_info.app_beat_info[max_index].beat_count) {
	...
	for (i = 0; i < TOP_APP_NUM; i++) {
		...
		} else if (top_app_beat_info.app_beat_info[max_index].beat_count >
				top_app_beat_info.app_beat_info[i].beat_count) {
			max_index = i;
		}
	}
...
static void oplus_app_monitor_update_top_push_app(struct app_monitor_app_info_st *app_info)
{
	...
	for (i = 0; i < TOP_APP_NUM; i++) {
		...
		} else if (top_app_push_info.app_push_info[max_index].push_count >
				top_app_push_info.app_push_info[i].push_count) {
			max_index = i;
		}
	}
...
static struct app_monitor_app_info_list_node_st *oplus_app_monitor_get_app_info_by_uid(u32 uid)
{
	...
	list_for_each_entry(app_info_node, &app_monitor_list[hash_index].m_listhead,
			    list_node) {
	...
	app_info_node = kzalloc(sizeof(struct app_monitor_app_info_list_node_st),
				GFP_ATOMIC);
...
void oplus_app_monitor_send_beat_alarm(u32 uid_val, u32 period)
{
	int ret;
	char msg_buf[1024] = {0};
	struct app_monitor_ul_beat_alarm_msg_st  *beat_alarm_msg =
		(struct app_monitor_ul_beat_alarm_msg_st *)msg_buf;
	...
	ret = apps_monitor_netlink_send_to_user(APPS_POWER_MONITOR_MSG_UL_BEAT_ALARM,
						(char *) msg_buf,
...
void oplus_app_monitor_send_push_alarm(u32  uid_val, u32 period)
{
	int ret;
	char msg_buf[1024] = {0};
	struct app_monitor_ul_push_alarm_msg_st  *push_alarm_msg =
		(struct app_monitor_ul_push_alarm_msg_st *)msg_buf;
	...
	ret = apps_monitor_netlink_send_to_user(APPS_POWER_MONITOR_MSG_UL_PUSH_ALARM,
						(char *) msg_buf, push_alarm_msg->msg_len);
...
void oplus_app_monitor_send_traffic_alarm(struct app_monitor_app_info_st *app_info)
{
	int ret;
	char msg_buf[1024] = {0};
	struct app_monitor_ul_traffic_alarm_msg_st  *traffic_alarm_msg =
		(struct app_monitor_ul_traffic_alarm_msg_st *)msg_buf;
	...
	ret = apps_monitor_netlink_send_to_user(APPS_POWER_MONITOR_MSG_UL_TRAFFIC_ALARM,
						(char *) msg_buf, traffic_alarm_msg->msg_len);
...
void oplus_app_update_sys_static_info(const struct sk_buff *skb, int send,
				      int retrans)
{
	if (send && retrans) {
		sys_stati_info.sys_retrans_total_packets++;
...
void oplus_app_monitor_update_app_info(struct sock *sk,
				       const struct sk_buff *skb, int send, int retrans)
{
	u32 uid_val;
	u32 hash_index;
	...
	if (send) {
		...
		if (app_samp_time >= PERIOD_CHECK_TIME && app_samp_time < REPORT_INTERVAL) {
			app_info_node->app_info.beat_period = (app_info_node->app_info.beat_period *
			...
			if (app_info_node->app_info.beat_count % ALARM_DETECT_COUNT == 0) {
				...
				if (app_info_node->app_info.beat_alarm_start_time
						&& time_gap < ALARM_DETECT_PERIOD) {
		...
		if (app_samp_time > PERIOD_CHECK_TIME && app_samp_time < REPORT_INTERVAL) {
			app_info_node->app_info.push_period = (app_info_node->app_info.push_period *
			...
			if (app_info_node->app_info.push_count % ALARM_DETECT_COUNT == 0) {
				...
				if (app_info_node->app_info.push_alarm_start_time
						&& time_gap < ALARM_DETECT_PERIOD) {
...
{
	struct app_monitor_dl_ctl_msg_st *dl_ctl_msg = (struct app_monitor_dl_ctl_msg_st
			*)NLMSG_DATA(nlh);
		...
		void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (write && ret == 0) {
		...
		oplus_app_monitor_send_beat_alarm(
			top_app_beat_info.app_beat_info[index].uid_val, (u32)app_beat_alarm_period);
		printk("[app_monitor]:alarm:uid=%u,beat_period=%d",
		       top_app_beat_info.app_beat_info[index].uid_val, app_beat_alarm_period);
	}
		...
		void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (write && ret == 0) {
		...
		oplus_app_monitor_send_push_alarm(
			top_app_push_info.app_push_info[index].uid_val, (u32)app_push_alarm_period);
		printk("[app_monitor]:alarm:uid=%u,push_period=%d",
		       top_app_push_info.app_push_info[index].uid_val, app_push_alarm_period);
	}
...
{
	app_power_monitor_table_hrd = register_net_sysctl(&init_net,
				      "net/oplus_app_power_monitor", app_power_monitor_sysctl_table);
...
void oplus_app_power_monitor_init(void)
{
	...
	memset(&top_app_traffic_info, 0,
	       sizeof(struct app_monitor_top_app_trafffic_info_st));
	...
	memset(&top_app_retrans_info, 0,
	       sizeof(struct app_monitor_top_app_retrans_info_st));
