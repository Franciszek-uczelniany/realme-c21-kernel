...
#ifdef VENDOR_EDIT//OPLUS_BUG_COMPATIBILITY
...
EXPORT_SYMBOL(sia81xx_volume_boost_power_get);

int sia81xx_volume_boost_power_set(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (sia81xx_audio_extern_config == 2)
...
EXPORT_SYMBOL(sia81xx_volume_boost_power_set);

int sia81xx_volume_boost_audio_scene_get(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (sia81xx_audio_extern_config == 2)
...
EXPORT_SYMBOL(sia81xx_volume_boost_audio_scene_get);

int sia81xx_volume_boost_audio_scene_set(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (sia81xx_audio_extern_config == 2)
...
EXPORT_SYMBOL(sia81xx_volume_boost_audio_scene_set);

int sia81xx_volume_boost_algo_en_get(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (sia81xx_audio_extern_config == 2)
	{
		...
		sia81xx_algo_en_read(g_sia81xx, &g_sia81xx->algo_en);

		ucontrol->value.integer.value[0] = g_sia81xx->algo_en;
	}
	return 0;
}
EXPORT_SYMBOL(sia81xx_volume_boost_algo_en_get);

int sia81xx_volume_boost_algo_en_set(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (sia81xx_audio_extern_config == 2)
	{
		...
		if (0 >= sia81xx_algo_en_write(g_sia81xx,
			ucontrol->value.integer.value[0]))
			return -EINVAL;
		g_sia81xx->algo_en = ucontrol->value.integer.value[0];
	}
	return 0;
}
EXPORT_SYMBOL(sia81xx_volume_boost_algo_en_set);
...
EXPORT_SYMBOL(sia81xx_volme_boost_set);
#endif /* OPLUS_BUG_COMPATIBILITY */
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node,
			"sia81xx_audio_extern_config", &sia81xx_audio_extern_config);
	pr_info("[ info][%s]  %s: line = %d  the PA type is  = %d\r\n", LOG_FLAG, __func__,__LINE__,sia81xx_audio_extern_config);
	if(0 != ret) {
		pr_err("[  err][%s] %s: get PA type return %d !!! \r\n",
			LOG_FLAG, __func__, ret);
		//return -ENODEV;
	}
#endif /* OPLUS_BUG_COMPATIBILITY */
