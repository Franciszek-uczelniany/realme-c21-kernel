...
extern  void oplus_set_otg_switch_status_dwc3(bool value);
extern  bool oplus_get_otg_online_status_dwc3(void);
extern  bool oplus_get_otg_switch_status_dwc3(void);
...
int smblib_disable_hw_jeita(struct smb_charger *chg, bool disable)
{
	 ...
	 */
/*#ifdef OPLUS_FEATURE_CHG_BASIC*/
/* Yichun.Chen  PSW.BSP.CHG  2019-03-28  disable jeita */
	...
	smblib_err(chg,"jeita reg 0x1090 = 0x%x\n", mask);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		oplus_chg_set_allow_switch_to_fastchg(true);
#endif
	}
 ...
 ***************/
#ifdef OPLUS_FEATURE_CHG_BASIC
extern wait_queue_head_t oplus_usbtemp_wq;
static BLOCKING_NOTIFIER_HEAD(msm_usb_notifier_list);
int msm_usb_register_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&msm_usb_notifier_list,	nb);
}
EXPORT_SYMBOL(msm_usb_register_client);

int msm_usb_unregister_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&msm_usb_notifier_list, nb);
}
EXPORT_SYMBOL(msm_usb_unregister_client);

int msm_usb_notifier_call_chain(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&msm_usb_notifier_list, val,  v);
}
EXPORT_SYMBOL(msm_usb_notifier_call_chain);
int oplus_set_dpdm_status(int status)
{
	int rc = 0;
	...
	rc = msm_usb_notifier_call_chain(POWER_SUPPLY_TYPEC_PLUGIN, NULL);
	return rc;
}
#define POWER_SUPPLY_TYPEC_PLUGIN 1
#define POWER_SUPPLY_TYPEC_PLUGOUT 0
...
static void oplus_ccdetect_work(struct work_struct *work)
{
	...
	if (level != 1) {
		...
		oplus_set_dpdm_status(POWER_SUPPLY_TYPEC_PLUGIN);
		if (oplus_usbtemp_check_is_support() == true)
			wake_up_interruptible(&oplus_usbtemp_wq);
	} else {
		oplus_set_dpdm_status(POWER_SUPPLY_TYPEC_PLUGIN);
		if (divider_in_auto_mode == false || (oplus_vooc_get_allow_reading() == false)) {
			cancel_delayed_work_sync(&chg->divider_set_work);
...
static void oplus_divider_set_work(struct work_struct *work)
{
	int rc;
	u8 stat;
	bool vbus_rising;
	...
	vbus_rising = (bool)(stat & USBIN_PLUGIN_RT_STS_BIT);
	smblib_err(chg, "oplus_divider_set_work vbus_rising=%d\n", vbus_rising);
	if (vbus_rising) {
...
static void oplus_dpdm_set_work(struct work_struct *work)
{
	if (g_oplus_chip && g_oplus_chip->vbatt_num != 2) {
		return;
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
    INIT_WORK(&chg->dpdm_set_work, oplus_dpdm_set_work);
...
static void oplus_ccdetect_enable(void)
{
    oplus_set_otg_switch_status_dwc3(true);
}

static void oplus_ccdetect_disable(void)
{    
	bool value = false;
	if(oplus_ccdetect_support_check() == OPLUS_SUPPORT_CCDETECT_IN_FTM_MODE)
	{
		printk(KERN_ERR "[OPLUS_CHG][%s]: ccdetect is in FTM mode, enable otg!\n", __func__);
		value = true;
	}
	if (g_oplus_chip && g_oplus_chip->vbatt_num == 2) {
		otg_disable_id_value();
	} else {
		oplus_set_otg_switch_status_dwc3(value);
	}
}
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (oplus_chg_is_usb_present() == true || usbtemp_volt <= USB_VBUS_SHORT_DISABLE_VOLT) {
			...
			wait_event_interruptible(oplus_usbtemp_wq, oplus_chg_is_usb_present() == true);
		}
...
static bool oplus_get_otg_switch_status(void)
{
	...
	if (oplus_ccdetect_check_is_gpio(chip) != true) {
        return oplus_get_otg_switch_status_dwc3();
    }
...
static int oplus_get_otg_online_status(void)
{
	...
	if (oplus_ccdetect_check_is_gpio(chip) == true) {
        ...
        return oplus_get_otg_online_status_dwc3();
...
static void oplus_set_otg_switch_status(bool value)
{
	...
	if (value) {
        ...
        oplus_set_dpdm_status(POWER_SUPPLY_TYPEC_PLUGOUT);
		oplus_ccdetect_disable();
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		if (g_oplus_chip != NULL && g_oplus_chip->vbatt_num == 2) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (of_find_property(oplus_chip->dev->of_node, "qcom,pm660usbtemp-vadc", NULL)) {
		oplus_chip->pmic_spmi.pm660_usbtemp_vadc_dev = qpnp_get_vadc(oplus_chip->dev, "pm660usbtemp");
...
#ifdef OPLUS_FEATURE_CHG_BASIC
    if (oplus_ccdetect_support_check() == OPLUS_NOT_SUPPORT_CCDETECT)
    	otg_disable_pmic_id_value();
#endif
