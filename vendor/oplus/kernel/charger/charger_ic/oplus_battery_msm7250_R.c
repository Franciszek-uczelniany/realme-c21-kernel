...
extern int oplus_ccdetect_gpio_init(struct oplus_chg_chip *chip);
extern void oplus_ccdetect_irq_init(struct oplus_chg_chip *chip);
extern void oplus_ccdetect_enable(void);
extern int qpnp_get_prop_charger_voltage_now(void);
extern int oplus_ccdetect_support_check(void);
...
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
void __attribute__((weak)) set_lpm_disallowed_flag(bool disable) {return;}
#endif
...
//#ifdef OPLUS_FEATURE_CHG_BASIC
			...
			pr_info("%s: work around usb charger type to dcp type\n", __func__);
			oplus_set_smb5_usb_props_type(apsd_result->pst);
			if (apsd_result->pst == POWER_SUPPLY_TYPE_USB_DCP)
			{
				pr_info("%s: notify usb charger type to dcp type\n", __func__);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	dev_err(chg->dev, "Set ship mode: %d!!\n", !!val->intval);
	msleep(1000);
#endif /* OPLUS_FEATURE_CHG_BASIC */
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if(g_oplus_chip->vbatt_num == 2){
		...
		if(((chg->svooc_detect_time - chg->svooc_detach_time) <= 1000) && oplus_vooc_get_detach_unexpectly() 
			...
			if(g_oplus_chip->svooc_disconnect_count < 3)
				g_oplus_chip->svooc_disconnect_count += 1;
			else
				oplus_vooc_set_disable_real_fast_chg(true);
			chg_err("g_oplus_chip->svooc_disconnect_count = %d\n",g_oplus_chip->svooc_disconnect_count);
		}else{
			g_oplus_chip->svooc_disconnect_count = 0;
			oplus_vooc_set_disable_real_fast_chg(false);
		}
		chg_err("chg->svooc_detect_time = %d, chg->svooc_detach_time = %d\n",chg->svooc_detect_time, chg->svooc_detach_time);
		chg->svooc_detect_time = 0;
	...
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: usbin-plugin %s\n",
					vbus_rising ? "attached" : "detached");
	chg_err("hvdcp is enable %d\n",oplus_hvdcp_is_enable(chg));
}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* set sink mode only */
	/*comment this codes for avoiding qd and qc detection issue during pd communitcation*/
	/*
	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
...
void oplus_set_otg_switch_status_default(bool value)
{
	int rc;
	u8 stat;
	bool vbus_rising;
	...
	{
		chg_err("ccdetect is gpio and return \n");
		return;
	}
...
//		if (oplus_usbtemp_check_is_support() == true)
	//		wake_up_interruptible(&oplus_usbtemp_wq);
		...
		if (oplus_ccdetect_get_power_role() != POWER_SUPPLY_TYPEC_PR_SINK
			...
			oplus_ccdetect_disable();
#ifdef OPLUS_FEATURE_CHG_BASIC
			if(chg->svooc_detach_time > 0 && g_oplus_chip->vbatt_num == 2){
				chg->svooc_detach_time = 0;
				oplus_vooc_set_detach_unexpectly(false);
				oplus_vooc_set_disable_real_fast_chg(false);
				chg_err("chg->svooc_detach_time =  %d, detach_unexpectly = %d\n",chg->svooc_detach_time, oplus_vooc_get_detach_unexpectly());
			}
#endif
...
int smbchg_get_chargerid_volt(void)
{
	...
	if (rc < 0) {
		chg_err("[OPLUS_CHG][%s]: iio_read_channel_processed  get error rc=%d\n", __func__, rc);
		return 0;
	}
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (rc < 0) {
		chg_err("[OPLUS_CHG][%s]: usbtemp_v_chan_r  get error\n", __func__);
		return ;
	}
	...
	if (rc < 0) {
        if(usbtemp_debug == 1)
		chg_err("[OPLUS_CHG][%s]: usbtemp_v_chan_l  get error\n", __func__);
        chip->usbtemp_volt_l = chip->usbtemp_volt_r;
        usbtemp_error_r = 1;
		return ;
	}
...
#endif /*OPLUS_FEATURE_CHG_BASIC*/
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!chg->pm7250b_bob_reg && of_get_property(node, "test_bob-supply", NULL)) {
		chg->pm7250b_bob_reg = devm_regulator_get(chg->dev, "test_bob");
		if (IS_ERR(chg->pm7250b_bob_reg)) {
			chg_err("Couldn't get pm7250b_bob regulator rc=%ld\n", PTR_ERR(chg->pm7250b_bob_reg));
			chg->pm7250b_bob_reg = NULL;
		}
	}
#endif /*OPLUS_FEATURE_CHG_BASIC*/
...
int pm7250b_bob_regulator_set_mode(unsigned int mode);

#endif

#ifdef OPLUS_FEATURE_CHG_BASIC
int pm7250b_bob_regulator_get_mode(unsigned int *mode)
{
	int rc;
	...
	if (!g_oplus_chip) {
		printk(KERN_ERR "pm7250b_bob_regulator_get_mode: g_oplus_chip NULL\n");
		return -1;
	}
	chg = &g_oplus_chip->pmic_spmi.smb5_chip->chg;

	if (!chg || !chg->pm7250b_bob_reg) {
		printk(KERN_ERR "%s: pm7250b_bob_reg NULL\n", __func__);
		return -1;
	}

	rc = regulator_enable(chg->pm7250b_bob_reg);
	if (rc < 0) {
		printk(KERN_ERR "%s: Couldn't enable regulator rc=%d\n", __func__, rc);
		return -1;
	}

	bob_mode = regulator_get_mode(chg->pm7250b_bob_reg);
...
err:
	rc = regulator_disable(chg->pm7250b_bob_reg);
	if (rc < 0) {
...
EXPORT_SYMBOL(pm7250b_bob_regulator_get_mode);
...
int pm7250b_bob_regulator_set_mode(unsigned int mode)
{
	...
	if (mode == REGULATOR_MODE_FAST) {
			ua_load = 2000000;
		rc = regulator_set_load(chg->pm7250b_bob_reg, ua_load);
		pr_err("pm8150a_bob set bob mode\n");
		if (rc < 0) {
		...
		ua_load = 0;
		rc = regulator_set_load(chg->pm7250b_bob_reg, ua_load);
		pr_err("pm8150a_bob set normal mode\n");
		if (rc < 0) {
			printk(KERN_ERR "%s: Couldn't set regulator load=%d rc=%d\n", __func__, ua_load, rc);
			return -1;
		}
...
EXPORT_SYMBOL(pm7250b_bob_regulator_set_mode);
...
/* Yichun.Chen  PSW.BSP.CHG  2019-08-19  for c to c */
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		if(((oplus_vooc_get_fastchg_started() == true) ||
			...
			(g_oplus_chip->vbatt_num == 1)&&(oplus_vooc_get_fast_chg_type() == FASTCHG_CHARGER_TYPE_UNKOWN)){
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		if(oplus_ship_check_is_gpio(g_oplus_chip) == false)
...
#ifdef OPLUS_FEATURE_CHG_BASIC
    rc = smblib_masked_write(chg, BATIF_ADC_CHANNEL_EN_REG,
                CONN_THM_CHANNEL_EN_BIT, 0);
    if (rc < 0)
        pr_err("Couldn't disable read conn_therm rc=%d\n", rc);
#endif
...
static void dump_regs(void)
{
	...
	if (!g_oplus_chip || !d_reg_mask || oplus_get_chg_powersave() == true)
		return;
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (g_oplus_chip && g_oplus_chip->enable_shipmode) {
		...
		if (oplus_ship_check_is_gpio(g_oplus_chip) == true) {
			...
			chg_debug("select pmic ship mode control\n");
			chg_err("ship mode enter,power off after 15s\n");
			msleep(1000);
			rc = smblib_masked_write(chg, SHIP_MODE_REG, SHIP_MODE_EN_BIT, SHIP_MODE_EN_BIT);
			pval.intval = 1;
			power_supply_set_property(chg->batt_psy,POWER_SUPPLY_PROP_SET_SHIP_MODE, &pval);
			if (rc < 0)
				chg_err("Couldn't enter ship mode, rc=%d\n",rc);
		}
	}
#endif /* OPLUS_FEATURE_CHG_BASIC */
}
